  ...
  for (let i = 0; i < arrayLenght; i++) {
    for (let j = 0; j < arrayLenght; j++) {
      for (let k = 0; k < arrayLenght; k++) {
        if (arr[i] + arr[j] + arr[k] === firstElement) {
          return true
        }
      }
    }
...
function MatchingCharacters(str) {
  ...
  let unique = 0
  for (var inputStrSize = str.length; inputStrSize > 2; inputStrSize--) {
    for (var i = 0; i <= str.length - inputStrSize; i++) {
      let inputString = str.slice(i, i + inputStrSize)

      if (inputString[0] == inputString.slice(-1)) {
        let betweenPairs = inputString.slice(1, -1)

        countUnique(betweenPairs, (uniqCount) => {
...
function countUnique(str, callback) {
  let myNewSet = new Set()
  for (var ch of str) {
    myNewSet.add(ch)
  }
  callback(myNewSet.size)
}
